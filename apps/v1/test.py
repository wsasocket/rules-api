...
class Hello(Resource):
    ...
    def get(self, name=None, id=None):
        ...
        test = r'alert tcp $EXTERNAL_NET any -> $SQL_SERVERS 3306 (msg:"SERVER-MYSQL UDF function drop attempt"; ' \
               r'flow:to_server,established; content:"|03|drop"; offset:4; nocase; content:"sys_"; within:50; ' \
               r'fast_pattern; pcre:"/\x03drop\s+function(\s+if\s+exists)*\s+sys_(exec|eval|get|bineval|set)/im"; ' \
               r'metadata:policy max-detect-ips drop, policy security-ips drop, service mysql; reference:url,' \
               r'github.com/rapid7/metasploit-framework/blob/master/modules/exploits/multi/mysql/mysql_udf_payload.rb' \
               r'; classtype:misc-activity; sid:45848; rev:1;) '
...
class Upload(Resource):
    ...
    def get(self):
        ...
        # 实际上应当是直接访问一个html，然后POST到这个接口，由于flask-restful的限制，
        # 可能会出现跨站请求的问题。
